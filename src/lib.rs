...
impl WasmLinearRegression {
    ...
    // Returns a value between -∞ and 1.0; closer to 1.0 is better.
    // Called from JS after fit() to display model quality.
    pub fn r2(&self, abalone: &WasmAbalone) -> f64 {
        let x = abalone.dataset.feature_matrix(); // flat row-major feature matrix
        let y = abalone.dataset.targets();         // ring counts as f64
        let m = abalone.dataset.len();             // number of samples
        let n = abalone.dataset.feature_names().len(); // number of features
        self.model.r2(&x, &y, m, n)
    }

    // RMSE over the full dataset — average prediction error in rings (same unit as target).
    // More interpretable than MSE: an RMSE of 2.1 means we're off by ~2.1 rings on average.
    // Called from JS after fit() to display alongside R².
...
pub fn wasm_grid_search(
    ...
    // Convert from JS typed array to Rust Vec — single conversion, used as a slice below
